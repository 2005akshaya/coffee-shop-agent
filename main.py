...
root_agent = Agent(
    ...
    instruction=(
        ...
        f'   - Ingest historical POS data from the "Sheet1" sheet tab using read_spreadsheet_values with spreadsheet_id="{SPREADSHEET_ID}".\n'
        ...
        '   - Analyze the historical POS data directly after reading it. Do NOT write or execute Python code for the analysis.\n'
        '   - The sandbox tool is only for simple shell operations when explicitly necessary; do not use it to perform spreadsheet-data analysis or generate large Python scripts.\n'
        '   - Correlate the 2025 product spikes (Cold Brew, Alt Milk, Extra Espresso) with the specific ceremonies ending at those times.\n'
        '   - Map those beverage profiles to the pasted schedule (which is the same sequence) to predict exactly when and where the 2026 spikes will occur.\n'
        '   - Identify expected wait-time bottlenecks in 2026 based on the 2025 wait times for those same profiles.\n'
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            events = list(events)  # materialize generator so it can be iterated more than once
            
            for event in events:
                print(f"[DEBUG EVENT FULL] {repr(event)}")
